...
3.  **透過管理端點觸發重置**：
    向 Parallax 的管理端點發送 POST 請求以重置熔斷器。**端點需 bearer token auth**（與其他非 `/healthz` 路由相同），缺 header 會回 401，並非端點壞掉。
    ```bash
    curl -X POST -H "Authorization: Bearer $PARALLAX_TOKEN" \
      http://<parallax-admin-endpoint>/admin/circuit_breaker/reset
    ```
    - 路徑使用底線 `circuit_breaker`，與 `parallax/server/routes/admin/circuit_breaker.py` 的 `APIRouter(prefix="/admin/circuit_breaker")` 一致；連字號版本不存在。
    - 預期收到 HTTP 200 + JSON `{"ok": true, "was_tripped": <bool>, "reset_at": "<iso8601>"}`。
