# Phase 4 PRD — Dual-Memory Router (DPKG × Parallax)

**狀態**: Draft
**日期**: 2026-04-18
**前置**: Phase 3 (Extract 層 + Shadow Write) 完成於 commit `82fe6b9`
**來源**: xcouncil 5-model 討論 + Opus 4.7 judge 收斂
**長期目標**: DPKG + Parallax 永久並存為雙記憶層，Engram 為未來升級點（非本階段）

---

## 1. 為什麼要做

Phase 3 把 Parallax 以 shadow-write 並行跑起來。現在**寫入雙軌**，但**讀取只走 DPKG** — 這不是永久架構，是過渡。
...
| semantic score / snippet / markdown | **DPKG 贏**，無條件 |
...
- [ ] DPKG adapter 實作 `SemanticRecallPort`（包 a2a ChromaDB 呼叫）
...
- [ ] `parallax/router/envelope.py` 定義統一回傳格式：
  ...
  {
    "results": [...],
    "authority": "parallax" | "dpkg" | "composed",
    "provenance": {...},
...
- [ ] 整合測試：real-ish flow with MockProvider + in-memory DPKG stub
...
- [ ] flag on：讀取走 `MemoryQueryRouter`；flag off：走原 DPKG 路徑
...
| DPKG adapter 跨 repo 依賴 a2a 內部 API | 定義穩定 adapter 介面，a2a 端實作，Parallax 不 import a2a 任何東西 |
...
**Parallax 是 state/provenance/timeline 的 SoT；DPKG 是 semantic recall 的 SoT；衝突時做欄位級仲裁並寫成 event，為 Engram 升級累積 migration 的 golden dataset。**
